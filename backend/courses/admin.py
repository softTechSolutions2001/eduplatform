...
from .constants import AccessLevel, CourseLevel, EnrollmentStatus, LessonType
from .models import (
    Answer,
    Assessment,
    AssessmentAttempt,
...
def validate_admin_permission(
    request: HttpRequest, required_permission: str = "view"
) -> bool:
    """
...
class CategoryAdmin(SecureAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Display Settings", {"fields": ("sort_order", "is_active", "featured")}),
        (
            "Timestamps",
    ...
    def save_model(self, request: HttpRequest, obj: Any, form: Any, change: bool):
        ...
        try:
            ...
            if Category.objects.filter(slug=obj.slug).exclude(pk=obj.pk).exists():
                messages.error(
                    request, f'Category with slug "{obj.slug}" already exists.'
                )
                return
...
class CourseAdmin(SecureAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Features", {"fields": ("has_certificate", "is_featured")}),
        ("Publishing", {"fields": ("is_published", "is_draft", "published_date")}),
        (
            "Version Management",
            {"fields": ("version", "parent_version"), "classes": ("collapse",)},
        ),
        (
            "Analytics",
            {
    ...
    def get_queryset(self, request: HttpRequest):
        ...
        if not request.user.is_superuser:
            ...
            if hasattr(request.user, "instructed_courses"):
                user_courses = request.user.instructed_courses.all().values_list(
                    "id", flat=True
                )
        ...
        return (
            queryset.select_related("category", "parent_version")
            .prefetch_related(
                Prefetch(
    ...
    def enrolled_students_display(self, obj):
        ...
        if count > 0 and validate_admin_permission(
            self.request if hasattr(self, "request") else None, "view"
        ):
            url = (
    ...
    def make_published(self, request: HttpRequest, queryset):
        ...
        if queryset.count() > MAX_BULK_OPERATIONS:
            messages.error(
                request,
                f"Cannot publish more than {MAX_BULK_OPERATIONS} courses at once.",
            )
            return
            ...
            with transaction.atomic():
                ...
                for course in queryset:
                    ...
                    if course.modules.count() == 0:
                        messages.warning(
                            request,
                            f'Course "{course.title}" has no modules and was not published.',
                        )
                        continue
                    ...
                    course.save(
                        update_fields=["is_published", "is_draft", "published_date"]
                    )
                    updated_courses.append(course.id)

                audit_admin_action(
                    request, "courses_bulk_published", "Course", updated_courses
                )
                messages.success(
                    request, f"{len(updated_courses)} courses published successfully."
                )
    ...
    def make_unpublished(self, request: HttpRequest, queryset):
        ...
        if queryset.count() > MAX_BULK_OPERATIONS:
            messages.error(
                request,
                f"Cannot unpublish more than {MAX_BULK_OPERATIONS} courses at once.",
            )
            return
    ...
    def make_featured(self, request: HttpRequest, queryset):
        ...
        try:
            ...
            audit_admin_action(
                request,
                "courses_bulk_featured",
                "Course",
                list(queryset.values_list("id", flat=True)),
            )
            messages.success(
                request, f"{updated} published courses marked as featured."
            )
    ...
    class Echo:
        ...
        # Add this method to CourseAdmin class:
        def export_course_data(self, request: HttpRequest, queryset):
            """Export course data with security validation - FIXED: Complete implementation"""
            if not validate_admin_permission(request, "view"):
                messages.error(
                    request, "You do not have permission to export course data."
                )
                return
            ...
            if queryset.count() > MAX_EXPORT_RECORDS:
                messages.error(
                    request,
                    f"Cannot export more than {MAX_EXPORT_RECORDS} courses at once.",
                )
                return

            # Only allow superusers to export
            if not request.user.is_superuser:
                messages.error(request, "Only superusers can export data.")
                return
            ...
            try:
                ...
                def generate_csv():
                    ...
                    # Write data rows
                    for course in queryset.select_related("category").prefetch_related(
                        "courseinstructor_set"
                    ):
                        instructors = ", ".join(
                            [
                                ci.instructor.get_full_name() or ci.instructor.username
                                for ci in course.courseinstructor_set.filter(
                                    is_active=True
                                )
                            ]
                        )
                        ...
                        yield writer.writerow(
                            [
                                ...
                                (
                                    course.get_access_level_display()
                                    if hasattr(course, "get_access_level_display")
                                    else course.access_level
                                ),
                                course.is_published,
                                course.created_date.strftime("%Y-%m-%d %H:%M:%S"),
                                instructors,
                                course.enrolled_students_count or 0,
                                course.avg_rating or 0,
                            ]
                        )

                # Generate CSV content
                csv_content = "".join(generate_csv())

                # Encrypt if encryption key is configured
                if (
                    hasattr(settings, "EXPORT_ENCRYPTION_KEY")
                    and settings.EXPORT_ENCRYPTION_KEY
                ):
                    try:
                        fernet = Fernet(settings.EXPORT_ENCRYPTION_KEY.encode())
                        encrypted_content = fernet.encrypt(csv_content.encode())
                        response = StreamingHttpResponse(
                            [encrypted_content], content_type="application/octet-stream"
                        )
                        response["Content-Disposition"] = (
                            'attachment; filename="courses_export.csv.encrypted"'
                        )
                    except Exception as e:
...
class ModuleAdmin(SecureAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Module Information", {"fields": ("course", "title", "description", "order")}),
        ("Settings", {"fields": ("is_published", "duration_minutes")}),
        (
            "Timestamps",
            {"fields": ("created_date", "updated_date"), "classes": ("collapse",)},
        ),
    ...
    def get_queryset(self, request: HttpRequest):
        ...
        return (
            queryset.select_related("course", "course__category")
...
class LessonAdmin(SecureAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("module", "title", "type", "order")}),
        (
            "Content",
            {
        ...
        ("Access Control", {"fields": ("access_level", "is_free_preview")}),
        ("Features", {"fields": ("has_assessment", "has_lab", "duration_minutes")}),
        ("Media", {"fields": ("video_url", "transcript"), "classes": ("collapse",)}),
        (
            "Timestamps",
            {"fields": ("created_date", "updated_date"), "classes": ("collapse",)},
        ),
    ...
    def get_queryset(self, request: HttpRequest):
        ...
        return queryset.select_related("module", "module__course").prefetch_related(
            "resources"
        )
