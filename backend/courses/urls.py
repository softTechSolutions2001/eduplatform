...
# Custom schema class for legacy endpoints
class LegacyEndpointSchema(AutoSchema):
    def __init__(self, operation_id_suffix=""):
        super().__init__()
        self.operation_id_suffix = operation_id_suffix

    def get_operation_id(self, path, method):
        operation_id = super().get_operation_id(path, method)
        if self.operation_id_suffix:
            operation_id = f"{operation_id}_{self.operation_id_suffix}"
        return operation_id


# Improved secure_endpoint decorator with schema support
def secure_endpoint(
    cache_timeout=None, require_auth=False, sensitive=False, operation_id_suffix=""
):
    ...
    def decorator(view_func_or_class):
        ...
        if inspect.isclass(view_func_or_class):
            ...
            result = method_decorator(
                throttle_classes((throttle_class, AnonRateThrottle)), name="dispatch"
            )(result)

            # Apply cache decorators if configured
            if cache_timeout:
                result = method_decorator(cache_page(cache_timeout), name="dispatch")(
                    result
                )
                result = method_decorator(
                    vary_on_headers("Authorization", "Accept-Language"), name="dispatch"
                )(result)

            # Apply custom schema for legacy endpoints
            if operation_id_suffix:
                result.schema = LegacyEndpointSchema(operation_id_suffix)
...
register_viewset(
    r"modules", views.ModuleViewSet, basename="module"
)  # FIXED: Use backward compatibility alias
...
urlpatterns = [
    ...
    # =====================================
    # User progress statistics - primary endpoint
    # Keep only the primary endpoint and update the legacy endpoint with unique operation_id
    # User progress statistics - primary endpoint
    path(
        "user/progress/stats/",
        require_http_methods(["GET"])(
            secure_endpoint(cache_timeout=CACHE_TIMEOUTS["user"], require_auth=True)(
                views.UserProgressStatsView.as_view()
            )
        ),
        name="user-progress-stats",
    ),
    # Legacy endpoint with unique operation_id using a different view class
    path(
        "user/progress-stats/",
        require_http_methods(["GET"])(
            secure_endpoint(cache_timeout=CACHE_TIMEOUTS["user"], require_auth=True)(
                views.LegacyUserProgressStatsView.as_view()
            )
        ),
    ...
    path(
        ...
        require_http_methods(["GET", "POST"])(
            secure_endpoint(require_auth=True, sensitive=True)(
                views.CourseEnrollmentView
            ).as_view()
        ),
    ...
    path(
        "search/",
        require_http_methods(["GET"])(
            secure_endpoint(cache_timeout=CACHE_TIMEOUTS["public"])(
    ...
    path(
        ...
        require_http_methods(["GET"])(
            secure_endpoint(cache_timeout=CACHE_TIMEOUTS["public"])(
                views.CertificateVerificationView
            ).as_view()
        ),
    ...
    path(
        ...
        require_http_methods(["GET"])(
            secure_endpoint(
                cache_timeout=CACHE_TIMEOUTS["analytics"], require_auth=True
...
- GET       /user/progress/stats/           - User progress statistics (primary)
...
OPERATION IDS:
- user_progress_stats_detailed              - Primary endpoint
- user_progress_stats_detailed_legacy       - Legacy endpoint
"""

# Log successful initialization
logger.info(
    f"Backward compatible security-optimized URLs configuration loaded - v7.3.3"
)
