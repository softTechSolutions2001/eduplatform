...
# - FIXED 🔴: Added select_for_update() for course cloning to prevent race conditions
# - FIXED 🔴: Improved exception handling to not swallow ValidationError
# - ENHANCED: Better transaction management and error handling
# - ADDED: Proper row-level locking for concurrent operations
...
class InstructorModuleViewSet(
    ...
    def create(self, request, *args, **kwargs):
        """Safe module creation with proper error handling"""
        try:
            ...
            # Don't catch ValidationError - let it propagate
            raise
        except Exception as e:
...
class InstructorDashboardView(
    APIView, ConsolidatedPermissionMixin, StandardContextMixin
):
    """
    ...
    def get(self, request):
        ...
        try:
            ...
            total_revenue = sum(
                course.enrollments.filter(status__in=["active", "completed"]).count()
                * float(course.price)
                for course in instructor_courses
            )
            ...
            dashboard_data = {
                "overview": {
                    ...
                    "avg_rating": float(
                        instructor_courses.aggregate(avg_rating=Avg("avg_rating"))[
                            "avg_rating"
                        ]
                        or 0
                    ),
    ...
    def get(self, request, course_slug):
        ...
        try:
            ...
            if all_lessons > 0:
                ...
                completion_rate = (
                    (
                        enrollment_stats["completed_students"]
                        / enrollment_stats["total_enrolled"]
                        * 100
                    )
                    if enrollment_stats["total_enrolled"] > 0
                    else 0
                )
            else:
            ...
            # Revenue analytics
            revenue_data = {
                "total_revenue": enrollment_stats["total_enrolled"]
                * float(course.price),
                "avg_revenue_per_student": float(course.price),
                "potential_revenue": (
                    enrollment_stats["active_students"]
                    + enrollment_stats["completed_students"]
                )
                * float(course.price),
            }
            ...
            analytics_data = {
                ...
                "progress_analytics": {
                    "avg_progress_percentage": round(
                        progress_records["avg_progress"] or 0, 1
                    ),
                    "completion_rate": round(completion_rate, 1),
                    "total_study_time_hours": round(
                        (progress_records["total_time"] or 0) / 3600, 1
                    ),
                },
    ...
    def delete(self, request, course_slug):
        ...
        try:
            ...
            with transaction.atomic():
                ...
                course.save(
                    update_fields=["is_published", "is_draft", "completion_status"]
                )
...
class CourseCloneView(APIView, ConsolidatedPermissionMixin, StandardContextMixin):
    """
    Clone an existing course with proper locking
    """
    ...
    def post(self, request, course_slug):
        """Clone a course with row-level locking"""
        try:
            # FIXED: Use select_for_update to prevent race conditions during cloning
            source_course = Course.objects.select_for_update().get(slug=course_slug)
            ...
            clone_options = {
                ...
                "copy_assessments": serializer.validated_data.get(
                    "copy_assessments", True
                ),
                "create_as_draft": serializer.validated_data.get(
            ...
            log_operation_safe(
                "Course cloned",
                f"{source_course.id} -> {cloned_course.id}",
                request.user,
            )
