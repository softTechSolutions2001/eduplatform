...
# Date Revised: 2025-06-30 09:45:13
# Current Date and Time (UTC): 2025-06-30 09:45:13
# Current User's Login: cadsanthanamNew
# Author: softTechSolutions2001
# Last Modified By: cadsanthanamNew
# Last Modified: 2025-06-30 09:45:13 UTC
...
class CategorySerializer(ContextPropagationMixin, OptimizedQueryMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'sort_order', 'parent', 'course_count', 'created_date', 'updated_date', 'featured'
        ]
...
class ResourceSerializer(ContextPropagationMixin, EnhancedValidationMixin, serializers.ModelSerializer):
    ...
    """
    # Rename to format_file_size to avoid collision with model property
    ...
    class Meta:
        ...
        fields = [
            ...
            'order', 'file_size', 'format_file_size', 'mime_type', 'uploaded',
            'storage_key', 'duration_minutes', 'format_duration',
    ...
    def get_format_file_size(self, obj):
        ...
        except (AttributeError, ValueError, TypeError) as e:  # Specific exceptions
...
class LessonSerializer(ContextPropagationMixin, OptimizedQueryMixin, EnhancedValidationMixin, serializers.ModelSerializer):
    ...
    def get_assessment(self, obj):
        ...
        try:
            context = self.get_serializer_context_for_nested()
            # Move the import to a function that returns the serializer class
...
class ModuleSerializer(ContextPropagationMixin, OptimizedQueryMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'duration_display', 'is_published', 'lessons', 'lessons_count',
...
class CourseSerializer(ContextPropagationMixin, OptimizedQueryMixin, EnhancedValidationMixin, serializers.ModelSerializer):
    ...
    class Meta:
        model = Course
        fields = [
            'id', 'title', 'subtitle', 'slug', 'description', 'short_description',  # Added short_description
            'category', 'category_id', 'thumbnail', 'price', 'discount_price', 'discount_ends',
...
class CourseDetailSerializer(CourseSerializer):
    ...
    def setup_eager_loading(cls, queryset):
        ...
        return super().setup_eager_loading(queryset).prefetch_related(
            ...
            Prefetch('reviews',
                    queryset=cls._get_review_queryset())
        )
    ...
    def get_recent_reviews(self, obj):
        ...
        try:
            ...
            if hasattr(obj, '_prefetched_objects_cache') and 'reviews' in obj._prefetched_objects_cache:
                ...
                from ..models import Review
                recent_reviews = Review.objects.select_related('user').filter(
                    course=obj,
                    is_approved=True
                ).order_by('-created_date')[:5]
